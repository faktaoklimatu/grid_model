...
class EmberNgLoader:
    ...
    def __init__(self,
                 ...
                 extrapolator: Optional[HourlyDataExtrapolator] = None) -> None:
    ...
    def get_pecd_normalization_factors(self,
        ...
        for (normalization_year, common_year) in zip(normalization_years, common_years):
            pecd_data_maps[normalization_year] = \
                self._extrapolator.load_country_year_pecd_data_map(
                    country, normalization_year, common_year)
    ...
    def get_installed(self,
                      ...
                      target_year: int,
                      country: Zone) -> dict[BasicSourceType, float]:
        assert self._extrapolator, "get_installed() needs an extrapolator"
        installed_gw_and_production_twh_map = \
            load_installed_and_production_from_ember_ng(self._df_sources,
                                                        scenario=scenario,
                                                        year=target_year,
        ...
        for (entsoe_year, common_year) in zip(self._entsoe_years, self._common_years):
            base_data: pandas.DataFrame = self._extrapolator.load_country_year_entsoe_data(
    ...
    def get_storage(self,
                    ...
                    target_year: int,
                    country: Zone,
                    allow_capex_optimization: bool = False) -> Collection[dict]:
        ...
        if self._load_hydro_from_pecd:
            storage += load_hydro_storage_from_pecd(
                self._extrapolator.get_pecd_parameters(
                    "PECD_EERA2021_reservoir_pumping_2030_country_table.csv"),
                self._extrapolator.get_pecd_parameters(
                    "PECD_EERA2021_ROR_2030_country_table.csv"),
                self._map_to_ember_region(country))
        return storage
    ...
    def get_country(self,
        ...
        result = {
            ...
            "pecd_normalization_factors": (self.get_pecd_normalization_factors(scenario,
                                                                               target_year, country)

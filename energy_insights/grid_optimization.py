...
from .sources.flexible_source import BackPressureTurbine, ExtractionTurbine, FlexibleSource
...
class CountryProblem:
    ...
    def __post_init__(self):
        ...
        if Keys.HYDRO_INFLOW_PUMPED_OPEN in self.grid.data:
            columns += [
                Keys.HYDRO_INFLOW_PUMPED_OPEN, Keys.HYDRO_INFLOW_RESERVOIR, Keys.HYDRO_INFLOW_ROR
            ]
    ...
    def _get_low_bound(self, installed, min_installed):
    ...
    def construct_country_problem(self,
        ...
        for t, (_index, row) in enumerate(self.grid.data.iterrows()):
            ...
            prob += total_supply_MW >= row[Keys.LOAD], self._make_name("Adequacy", t)
            ...
            for index, storage_type in enumerate(self.grid.storage):
                ...
                if storage_type.min_capacity_mw < storage_type.capacity_mw:
                    ...
                    discharging_installed_factor = self.storage_discharging_installed_list[index]
                    prob += (
                        discharging_MW <= storage_type.capacity_mw * discharging_installed_factor,
        ...
        # production.
        for index, flexible_source in enumerate(self.grid.flexible_sources):
            if flexible_source.max_total_twh is not None:
                flexible_production_el_eq_MWh_sum = flexible_production_el_eq_sum_MWh_list[index]
                max_total_twh = flexible_source.max_total_twh * self.grid.num_years
                prob += (flexible_production_el_eq_MWh_sum / 1e6 <= max_total_twh,
    ...
    def extract_solution(self) -> None:
        ...
        df["Production"] = df["VRE"] + df[Keys.HYDRO] + df[Keys.NUCLEAR] + df["Flexible"]
        df["Total_Without_Storage"] = df["Production"] + df[Keys.NET_IMPORT]

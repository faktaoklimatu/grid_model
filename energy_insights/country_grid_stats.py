...
class StatType(Enum):
    ...
    WHOLESALE_REVENUES_PPA_MN_EUR = "wholesale_revenues_PPA_mn_EUR"
...
class CountryGridStats:
    ...
    def _compute_emissions(self, season: Season) -> None:
        productions_twh = self.get_stat_dict(StatType.PRODUCTION_TWH, season)

        for production_twh in productions_twh.values():
            source = production_twh.source
            # This excludes the aggregate value.
            if source not in (CountryGridStats.import_export, CountryGridStats.total):
                # t per MWh = Mt per TWh.
                co2_Mt = production_twh.val * source.co2_t_mwh
                self._store_stat(source, StatType.EMISSIONS_MTCO2, co2_Mt, season)
            # TODO: Calculate emissions from heat production in CHP.

        self._store_stat(CountryGridStats.total, StatType.EMISSIONS_MTCO2,
                         sum(self.get_stat_values(StatType.EMISSIONS_MTCO2)), season)
